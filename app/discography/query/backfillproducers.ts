...
export async function releasesBackfillProducers(arg1?: string, arg2?: string): Promise<void> {
    const {limit, retryBlanks, invalidArg} = parseArgs(arg1, arg2);
    if (invalidArg) {
        console.log(`[releases.producers.backfill] invalid argument "${invalidArg}"`);
        return;
    }

    const pending = await discography.getReleasesPendingProducerBackfill(limit ?? undefined, retryBlanks);
    console.log(
        `[releases.producers.backfill] pending=${pending.length}${limit ? ` limit=${limit}` : ""}${retryBlanks ? " retry_blanks=true" : ""}`,
    );
    ...
    for (let index = 0; index < pending.length; index++) {
        ...
        const prefix = `[releases.producers.backfill] ${index + 1}/${pending.length}`;
        ...
        try {
            ...
            const producer = (hydrated.producer || "").trim();
            await discography.saveReleaseProducerById(release.id, producer);
            updated += 1;

            if (!producer) {
                blank += 1;
                console.log(`${prefix} status=saved producer=(blank)`);
            } else {
                console.log(`${prefix} status=saved producer=${producer}`);
            }
        } catch (e) {
            failed += 1;
    ...
    console.log(
        `[releases.producers.backfill] complete updated=${updated} blank=${blank} failed=${failed}`,
    );
}
